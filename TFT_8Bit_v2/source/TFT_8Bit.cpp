...
TFT::TFT(uint16_t x, uint16_t y, TIM_HandleTypeDef *TFT_Timer){
	WIDTH = x;
	HEIGHT = y;
	Timer = TFT_Timer;
}
...
uint16_t TFT::width(void){ return _width; }
uint16_t TFT::height(void){ return _height; }
...
void TFT::ReadGRAM(int16_t x, int16_t y, uint16_t block[], int16_t w, int16_t h){
	uint8_t r, g, b;
	uint16_t color;
...
uint16_t TFT::ReadPixel(int16_t x, int16_t y){
	uint16_t GRAM[1];
...
void TFT::vertScroll(int16_t top, int16_t scrollines, int16_t offset){
#if defined(OFFSET_9327)
	if (_lcd_ID == 0x9327) {
...
void TFT::pushColors(uint16_t * block, int16_t n, uint8_t first){
    pushColors_any(_MW, (uint8_t *)block, n, first, 0);
}
void TFT::pushColors(uint8_t * block, int16_t n, uint8_t first){
    pushColors_any(_MW, (uint8_t *)block, n, first, 2);   //regular bigend
}
void TFT::pushColors(const uint8_t * block, int16_t n, uint8_t first, uint8_t bigend){
    pushColors_any(_MW, (uint8_t *)block, n, first, bigend ? 3 : 1);
}
void TFT::fillScreen(uint16_t color){
    fillRect(0, 0, _width, _height, color);
}
void TFT::invertDisplay(uint8_t i){
    uint8_t val;
...
void TFT::writeLine(int16_t x0, int16_t y0, int16_t x1, int16_t y1, uint16_t color) {
...
void TFT::fillCircleHelper(int16_t x0, int16_t y0, int16_t r, uint8_t corners, int16_t delta, uint16_t color){
    int16_t f     = 1 - r;
    int16_t ddF_x = 1;
    int16_t ddF_y = -2 * r;
    int16_t x     = 0;
...
void TFT::WriteByteBack(uint16_t x, uint16_t y, uint16_t color, uint16_t B_color, uint8_t Char){
...
void TFT::WriteByteNoBack(uint16_t x, uint16_t y, uint16_t color, uint8_t Char){
   for(uint8_t i=0; i<8; i++){
		if(Char&(0x01)) {
...
void TFT::WriteNByteBack(uint16_t x, uint16_t y, uint16_t color, uint16_t B_color, uint8_t Char){
...
void TFT::WriteNByteNoBack(uint16_t x, uint16_t y, uint16_t color, uint8_t Char){
...
void TFT::WriteCharBack(uint8_t numWrite, uint16_t x, uint16_t y, uint16_t color, uint16_t B_color, const uint8_t *font, uint16_t num){
...
void TFT::WriteCharNoBack(uint8_t numWrite, uint16_t x, uint16_t y, uint16_t color, const uint8_t *font, uint16_t num){
	for(uint8_t i = 0; i < numWrite; i++){
		WriteByteNoBack(x, y+i*8, color, font[num+i]);
	}
}
void TFT::drawChar(uint16_t x, uint16_t y, const G_Font *font, bool back, uint16_t color, uint16_t B_color, uint8_t Char){
	uint16_t i = x, j = y;
	gfxFont = font;
	W = gfxFont -> w;
	H = gfxFont -> h;
...
void TFT::print(uint16_t x, uint16_t y, const G_Font *font, bool back, int16_t N_Color, int16_t B_Color, char *str) {
	uint16_t i = x;
...
void TFT::print(uint16_t x, uint16_t y, const G_Font *font, bool back, int16_t N_Color, int16_t B_Color, float Num) {
	char c[10];
	sprintf(c, "%5.3f", Num);
    print(x, y, font, back, N_Color,B_Color, c);
}
void TFT::print(uint16_t x, uint16_t y, const G_Font *font, bool back, int16_t N_Color, int16_t B_Color, long Num) {
	 char c[20];
     print(x, y, font, back, N_Color,B_Color, itoa(Num,c,10));
}
void TFT::drawBitmap(uint16_t x, uint16_t y, const uint8_t* bitmap, bool back, uint16_t color, uint16_t B_color, uint16_t w, uint16_t h) {
...
//    if (is555) color = color565_to_555(color);
	...
	void TFT::drawRGBbitmapInFlash(W25Q w25q, uint32_t Sector_OffSet_Addr, uint16_t x, uint16_t y, uint16_t w, uint16_t h) {
...
#endif
#ifdef USE_SD_CARD
	void TFT::drawBitmapFile(char *s, uint16_t x,uint16_t y, uint16_t s_x,uint16_t s_y, uint16_t Width, uint16_t Height, uint16_t file_Width, uint16_t file_Height){
		 uint16_t i,h;
  ...
  #ifdef USE_EXTERNAL_FLASH
	void TFT::CopyBmpFileToFlash(W25Q w25q, char *s, uint32_t Sector_OffSet_Addr, uint16_t s_x,uint16_t s_y, uint16_t Width, uint16_t Height, uint16_t file_Width, uint16_t file_Height){
  ...
  #endif
#endif
uint8_t TFT::getRotation (void){return rotation;}
void TFT::scrollup (uint16_t speed){
     uint16_t maxscroll;
...
void TFT::scrolldown (uint16_t speed){
	uint16_t maxscroll;
	if (getRotation() & 1) maxscroll = width();
	     else maxscroll = height();
	for (uint16_t i = 1; i <= maxscroll; i++){
